...
def draw_ocr_box_txt(image, boxes, txts, scores=None, drop_score=0.3, font_path="./simfang.ttf") -> np.array:
    ...
    for idx, (box, txt) in enumerate(zip(boxes, txts)):
        ...
        if box_height > 2 * box_width:
            # 垂直方向文字框
            font_size = max(int(box_width * 0.6), 15)
            font = ImageFont.truetype(font_path, font_size, encoding="utf-8")
            cur_y = box[0][1]
            for c in txt:
                char_size = font.getsize(c)
                draw_right.text(
                    (box[0][0] + 3, cur_y), c, fill=(0, 0, 205), font=font)
                cur_y += char_size[1]
        else:
            # 水平方向文字框
            font_size = max(int(box_height * 0.6), 15)
            font = ImageFont.truetype(font_path, font_size, encoding="utf-8")
            draw_right.text(
